...
# note index 277,348,681.Dates are 15th,24th,21st
 #We will have to remove these indices after merging stock and news data
df = df.dropna(axis=0)
df.isnull().sum()
...
type(traindataset)
# Its a sparse matrix which is expected since every row 
#will contain only few words. For those words there is a 1
# else zero. So its a sparse matrix
...
print(cv1)
# Here we see column numbers and occurence of that word 
# in 1st message
# 120724th column sees word coming twice in news1
# This means that there are 10 unique words in news1. 
#One of them appear twice, the rest only once.
...
# Weights have been assigned to each word not only in that
#message but according to whole corpus i.e. what significance
#does a word hold in whole dataset(TD*IDF)

# trying tf-idf on news1
tfidf1 = tfidf.transform(cv1)
print(tfidf1)
# weights have been assigned to each word wrt whole dataset
# Help us(135029) has weight of 0.46 wrt whole dataset
#(TF*IDF)
...
#applying RFOREST on whole dataset
